...
# Everything that touches the system lives in main(), and main runs only from the
# last line of the file. README.md documents `curl ... | sh`, so a connection that
# drops mid-transfer feeds sh a truncated script: without this, the prefix that did
# arrive executes and leaves a half-installed system instead of a failed install.
# Top level keeps only `set -eu`, two constants, and function definitions -- none of
# which change anything if they are all that arrives.
main() {
