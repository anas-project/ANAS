...
func TestOpenAPITracksImplementedSurface(t *testing.T) {
	...
	wantResponses := map[string][]string{
		...
		"GET /api/v1/system":                                              {"200", "400", "404", "405", "408", "500", "504"},
	...
	for _, name := range []string{
		...
		"ConflictProblem", "PreconditionProblem", "GetMethodNotAllowedProblem", "PostMethodNotAllowedProblem",
		...
		"RequestCanceledProblem", "InternalProblem", "ServiceUnavailableProblem", "DeadlineProblem",
	} {
...
func TestOpenAPIAuthenticationContractMatchesHandlers(t *testing.T) {
	...
	if bootstrapToken["writeOnly"] != true || handoffToken["writeOnly"] != true || enrollmentPassword["writeOnly"] != true || localPassword["writeOnly"] != true {
		t.Errorf("credential request fields must be writeOnly: token=%#v handoff=%#v enrollment_password=%#v local_password=%#v", bootstrapToken, handoffToken, enrollmentPassword, localPassword)
	}
