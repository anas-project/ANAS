...
type backupOptions struct {
	...
	yes        bool
	json       bool
	// skipUserData makes a deployment-only backup. The default is to include
	// user content, because a backup exists so that a failed disk loses
	// nothing, and the files people put there are the part that cannot be
	// regenerated by redeploying. This is the opposite default from snapshots,
	// deliberately: a snapshot exists to undo a deploy, a backup to survive
	// hardware.
	skipUserData bool
}
...
func prepareBackupSource(workspace, base string, plan *backupPlan, opts backupOptions) (*backupSource, string, int, error) {
	...
	meta, err := createSnapshot(workspace, snapshotOptions{
		...
		// createSnapshot captures what it can and records what it could not, so
