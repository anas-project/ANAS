...
func TestEveryCommandEmitsOneDocumentAndTheDocumentedExitCode(t *testing.T) {
	...
	cases := []struct {
		...
		{"credential list with no active deployment", []string{"credential", "list", "-w", workspace, "--json"}, 4},
		{"credential dry-run with no active deployment", []string{"credential", "rotate", "missing", "-w", workspace, "--dry-run", "--json"}, 4},
