...
> [!NOTE]
> 这是已实施功能的设计与迁移记录，正文中的“当前”指制定计划时的历史基线。现行目录、命令和恢复语义以[备份与恢复指南](../../../docs/guide/backup-and-restore.md)及[备份契约](../../../docs/reference/contracts/backup.md)为准。

> 状态：一（workspace 语义）、二（`anas init`）、三（snapshot 体系）、
> 四（breaking 升级自动快照）、五（backup 体系）已落地。本文是总纲，JSON 契约见
> [CLI 契约](../../../docs/reference/contracts/index.md)，module 分发见
> [module-distribution-draft.md](../../../docs/architecture/module-distribution-draft.md)（草案，不在本计划内）。
...
| [runtime-release-state-design.md:134](../../../docs/architecture/runtime-release-state-design.md)、§13 | §4「推荐运行目录」画的仍是 `~/.anas/` 树，§13 备份单元仍是五项清单，两处均与本计划冲突，随代码同步更新 |
...
完整契约见 [snapshot 契约](../../../docs/reference/contracts/snapshot.md)，实现见
...
完整契约见 [backup 契约](../../../docs/reference/contracts/backup.md)，实现见
...
见 [CLI 契约](../../../docs/reference/contracts/index.md)：结构化结果走 stdout 且只有一个 JSON
...
- **代价是状态权威分裂。** 本设计建立在"磁盘文件是唯一权威"之上（见
  [runtime-release-state-design.md](../../../docs/architecture/runtime-release-state-design.md) §3），常驻进程天然
  倾向于在内存里攒状态并与磁盘漂移。
...
守护进程。它以 root 运行、持有同一把 `state/lock`、读写同一批文件；[CLI 契约](../../../docs/reference/contracts/index.md)
...
第五期可与第四期合并。[module 分发](../../../docs/architecture/module-distribution-draft.md)不在本计划内。
