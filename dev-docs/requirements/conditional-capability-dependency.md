...
锁和 `plan` 输出中的行为。实施进度见[条件 Capability 依赖实施计划](../plans/conditional-capability-dependency.md)。
Capability 的通用规则见[Capability 开发标准](../../docs/developer/capability-development.md)。
...
- **给 `requires_capabilities` 加 request schema，让 Consumer 声明它需要的准入策略。** 这是把
  Capability 改造成 Contract。[Capability 开发标准](../../docs/developer/capability-development.md) §1 的 ABI 硬
