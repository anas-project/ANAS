...
> [cli.go](../../internal/runner/cli.go)；分发与统一错误封装见
> [runner.go](../../internal/runner/runner.go) 的 `Main`。
...
- [internal/runner/contract_test.go](../../internal/runner/contract_test.go)
  表驱动走一遍所有不需要真实部署的命令：stdout 恰好一个文档、信封字段齐全、
  退出码逐个断言到具体数字。
- [test-env/scripts/test-contract.sh](../../test-env/scripts/test-contract.sh)
