...
Providers are pinned to Authentik `2026.5.6`, LLNG `2.23.2`, and Casdoor `3.143.0`. Both SAML HTTP-POST and Redirect are browser bindings; the word `post` never implies browserless revocation. Casdoor registers only explicitly declared OIDC back-channel URIs and clears old values when declarations disappear or protocols switch; user/admin exact-`sid` notification passed real standards-consumer E2E. The pinned version has no SAML SLO consumer path and publishes no SLO.
