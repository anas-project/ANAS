...
*[English version](/en/guide/usage)*

这是面向任务的指南：怎么建起一套部署、怎么日常运行、怎么改、出问题怎么救回来。
每条命令产出的确切 JSON 与退出码见 [CLI JSON 契约](/reference/contracts/)。
...
- `.anas/` 绝大部分可重建。线上实测 402 MB，其中真正不可重建的只有 **52 KB**
  （`state/` 和 `secrets.yml`），活动制品再加 42 MB，剩下约 360 MB 是历史
  deployment 和构建缓存。
...
恢复出的 workspace 自带制品，因此**不需要 module 源码树**就能启动。目标主机仍需
能获取冻结 Compose 引用的运行镜像，除非这些镜像已经存在于本机。
...
完整细节见 [CLI JSON 契约](/reference/contracts/)。
