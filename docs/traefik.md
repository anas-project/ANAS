...
* `- "traefik.http.middlewares.auth.basicauth.users=${TRAEFIK_BASICAUTH_HTPASSWD}"`: 由本 module 的 hook 生成，用户名取 `BASICAUTH_USER`（默认 **admin**），密码取 `BASICAUTH_PASSWD`，未设置时用该 module 的 `DEFAULT_SERVICE_ROOT_PASSWORD`。哈希格式为 `{SHA}`；Traefik 也接受 bcrypt，但 bcrypt 每次加盐都会产生新值，而这个值渲染进 Compose label，会导致每次 render 都重建容器——要换算法需要同时把哈希本身存进 secret store。也可以在配置的 `env:` 里直接给出现成的 htpasswd 行（`TRAEFIK_BASICAUTH_HTPASSWD`），此时不再推导。
