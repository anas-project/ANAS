...
[snapshot 契约](../reference/contracts/snapshot.md)。
...
- release 普通资产在封印后改为只读（`sealDeployment`，rename 进
  ...
  见 [snapshot 契约](../reference/contracts/snapshot.md)；
...
- **备份单元就是整个 `<workspace>/`**。这正是 workspace 布局存在的理由：把它拆成
  ...
  其余不行；详见 [backup 契约](../reference/contracts/backup.md)；
...
2. 修复 `config secret list -w ...` 参数解析；
