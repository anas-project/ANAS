...
| 模块 | 当前持久状态 | 判断与处理建议 |
| --- | --- | --- |
| runner 自身 | workspace 配置、生成 Secret、`config.lock.yml`、deployment 制品和 active state | Secret 是关键状态；lock 固化解析；deployment 是不可变执行制品。 |
| `collabora` | 无 | 无关键本地状态。 |
| `ddns_go` | `${DATA_PATH}/ddns_go:/root` | 配置和运行历史已持久化。 |
| `ddns_updater` | `/updater/data` 未挂载 | 当前主要损失是历史/缓存；若依赖更新历史，应声明为可选运行状态。 |
| `eturnal` | `TURN_SECRET` 在 generated secrets | 已满足稳定密钥要求，无业务卷。 |
| `freeradius` | 无 | 当前只是 experimental scaffold；正式启用前必须声明证书、客户端密钥和计费数据的存储。 |
| `authentik` | `${DATA_PATH}/authentik/data` 加外部数据库 | 当前为 experimental；数据库和持久目录必须一致备份，bootstrap 凭据需要明确轮换流程。 |
...
| `netbird` | signal 与 management 数据目录 | 功能仍为 experimental；正式使用前还需验证完整备份与恢复流程。 |
...
| `.anas/state/` | active deployment 和运行状态 | 必须由 ANAS 管理，不手工修改 |
...
| Authentik | bootstrap 管理员凭据 | bootstrap 资料只在初始化阶段生效 | Authentik 管理 API/CLI 轮换 |
| Authentik/LLNG/Nextcloud | SSO entity、issuer、redirect URI 与域名 | 部分配置能重渲染，但已有客户端/元数据/外部信任不会自动整体迁移 | 跨模块 reconcile，保留过渡地址和签名验证窗口 |
...
- SSH 私钥/公钥；
- TURN secret；
- LLNG/Authentik 的 SAML/OIDC 私钥、证书和 key ID；
...
历史第一阶段曾使用独立 `config-applied.yml` 保存显式配置指纹。现行 deployment 已冻结 guarded settings；下面保留的是更完整 observed state 的提案示意：
...
1. 数据库、Samba service account、Nextcloud/Authentik admin 的凭据轮换事务。
