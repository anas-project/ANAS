...
Hook、渲染和启动各阶段的边界。实施进度见[无序 Capability 依赖实施计划](/plans/weak-capability-dependency)。
条件依赖（`enabled_by`）见[条件 Capability 依赖要求](/requirements/conditional-capability-dependency)，
两者可以叠加。
...
（[Capability 开发标准](/developer/capability-development) §1 硬边界第 5 条），不执行任何持久对象的
生命周期。
