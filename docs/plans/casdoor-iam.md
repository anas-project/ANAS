...
| M3：删除/停用、Group 门禁、永久锚点与 OIDC/SAML 登录 | R-015—R-016、R-019、R-022—R-025 | 未开始 |
| M4：OIDC 会话撤销与 SAML SLO | R-027—R-029、R-031 | 未开始 |
| M5：恢复、备份、升级、密钥轮换与发布 | R-034—R-039 | 未开始 |

## 2. 已完成落地快照

- M1 已固定 Casdoor `3.143.0-r3`，接入 PostgreSQL Resource、Traefik、OIDC/SAML registration、
...
- 当前实现明确不执行 `ALLOW_GROUPS`，不发布未经验证的 SAML SLO，也不把 `$user.id` 声称为永久
  Samba 锚点；这些限制归入 M3/M4，不按已完成能力统计。

## 3. M3 实施检查表

- [ ] 扩展目录同步，使删除、禁用和组移除在 Casdoor 本地影子记录及新凭据签发上确定性收敛。
- [ ] 为通用 `ALLOW_GROUPS` 建立 Casdoor 可证明等价的 Application/Permission/Role 策略；若固定版本
  无法表达，则停止发布级评估并回到需求范围决策。
- [ ] 让 OIDC 与 SAML 使用可由 Samba `anasIdentityAnchor` 证明的稳定身份映射，覆盖改名不重复建号。
...
| R-015 | 待新增 `test-env/scripts/server-casdoor-directory-authority-e2e.sh` | Samba AD + Casdoor，隔离 Docker daemon | — | 待实现：删除、停用与凭据拒绝 |
| R-016 | 待新增 `test-env/scripts/server-casdoor-directory-authority-e2e.sh` | Samba AD + Casdoor，隔离 Docker daemon | — | 待实现：直接组、递归组、加入与撤权传播 |
...
- Casdoor 固定版本尚无已证明等价于通用 `ALLOW_GROUPS` 的策略映射，M3 在该能力落地前不能完成。
- 当前 LDAP 导入保留本地影子用户，删除、停用和 Group 撤权的最终状态尚无真实 E2E 证据。
- Samba 永久锚点尚未成为 OIDC/SAML 都可验证的稳定 subject/NameID，用户改名连续性未验收。
