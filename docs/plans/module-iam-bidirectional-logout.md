...
验收依据是[使用 OIDC/SAML 的 Module 双向登出要求](/requirements/module-iam-bidirectional-logout)
的 `LOGOUT-R-###` 矩阵。固定版本能力结论见
[Module IAM / OIDC 支持清单](/reference/module-iam-support#固定版本登出矩阵)。实现已经落地；当前里程碑
