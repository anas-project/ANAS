...
验收依据是[条件 Capability 依赖要求](/requirements/conditional-capability-dependency)的需求矩阵。
本主题没有独立架构文档——它扩展的是既有 Capability 解析路径，不引入新的运行时边界；与应用目录角色
模型的关系写在[应用目录设计](/architecture/app-catalog-design) §4.1。

M0、M1、M2 全部完成。M2 曾被一个结构性依赖环阻塞（`oauth2_proxy` 需要 IAM，IAM 需要数据库，而那正是
要守卫其 Adminer 的 Module），由[无序 Capability 依赖](/plans/weak-capability-dependency)解除。
...
本轮完整接线过一遍，因依赖环回退；环由[无序 Capability 依赖](/plans/weak-capability-dependency)的
