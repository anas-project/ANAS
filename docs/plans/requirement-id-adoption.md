...
- [ ] 迁移完成后，把 `requirementsDir` / `plansDir` 常量一次性改到新位置，不保留旧路径回退。
- [ ] 扩展扫描逻辑覆盖 Module 私有需求目录，配对规则与仓库级一致。
- [ ] 新增测试：同一主题在新旧两个位置同时存在时门禁必须失败。
- [ ] 通过 `npm run docs:check-requirements`。
...
M2 落地时需要同步改本生成器的 `requirementsDir` / `plansDir` 常量，与门禁脚本一并切换。
