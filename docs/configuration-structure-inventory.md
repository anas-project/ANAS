...
| `authentik` | 7 | `db_name`, `db_type`, `domain_prefix`, `image_tag`, `ldap_enabled`, `ldap_password_writeback`, `log_level` |
...
| `ddns_updater` | 8 | `dns_provider`, `domain_prefix`, `forward_auth_interface`, `publicip_dns_providers`, `publicip_fetchers`, `publicip_ipv4_providers`, `publicip_ipv6_providers`, `publicip_providers` |
| `eturnal` | 2 | `domain_prefix`, `port` |
| `lam` | 2 | `admin_password`, `domain_prefix` |
...
| `meshcentral` | 2 | `domain_prefix`, `mps_port` |
| `netbird` | 3 | `adminer_enabled`, `domain_prefix`, `iam_protocol` |
| `nextcloud` | 13 | `admin_password`, `db_name`, `db_type`, `debug`, `domain_prefix`, `iam_protocol`, `log_level`, `memories_enabled`, `memory_limit`, `phone_region`, `rm_skeleton_files`, `talk_enabled`, `upload_max_size` |
...
| `DOCKER_HUB_REGISTRY` | Docker Hub 镜像前缀 |
| `LLNG_DOCKER_HUB_REGISTRY` | LemonLDAP::NG 专用 Docker Hub 镜像前缀 |
| `ANAS_IMAGE_REGISTRY` | ANAS 自建镜像仓库 |
| `GHCR_REGISTRY` | 第三方 GHCR 镜像前缀 |
| `QUAY_REGISTRY` | Quay 镜像前缀 |
