...
以下是最常用的一组命令。第 1 节只保留操作路径；Docker、Samba 用户和组参数分别在后面的唯一章节解释，避免同一参数维护多份说明。默认 Samba DC 容器名为 `anas_samba_dc`；它受 [`global.container_prefix`](/reference/configuration#已声明的-139-个参数) 影响。自定义部署先按 [§2 操作前检查](#_2-操作前检查) 找到实际容器名。
