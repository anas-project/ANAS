...
created: 2026-08-22
updated: 2026-08-23
evidence_as_of: 2026-08-23
---

# ANAS 审计整改任务清单

本文的前身是一份全面审计报告。2026-08-23 逐条复核后改写成任务清单：**每条结论都重新对照代码验证
过，不成立的已移到 §3，已修复的已移到 §2，剩下的写成可以独立执行的任务。**
...
每个任务是自足的：给出**定位**（文件和行号）、**现状**（复核时看到的事实）、**要做什么**、
**不要做什么**、**验证**和**风险**。在一个全新会话里只读某一个任务应当就能执行，不需要回看审计
上下文。

任务之间的依赖在标题后标注。没有标注的可以任意顺序做。

### 验证基线

任何任务完成后至少跑这一组。2026-08-23 复核时全部通过：
...
- `anasd` 目前是**纯只读** HTTP API：`internal/api/httpapi/handler.go` 对所有路由强制 GET。这一点
  是若干任务优先级判断的前提，见 §5。
