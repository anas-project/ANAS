...
// The calculate environment is shared with every cask that has this one in its
...
// render_env is cask-local, so the container config script gets its key there.
