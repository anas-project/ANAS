...
# would have no certificate at all during that window. Each cask used to paper
# over that by generating its own self-signed certificate, which is why nothing
# trusted anything: there were as many issuers as there were casks.
#
# The CA private key lives beside the ACME account under LEGO_DATA_PATH and
# never enters the shared certificates/ directory that other casks mount, the
