...
| `forgejo.actions_incus_client_cert_b64` | string | — | `""` | `static` | `FORGEJO_ACTIONS_INCUS_CLIENT_CERT_B64` | no | no | yes | yes | `container_recreate` | Base64 restricted-project Incus TLS client certificate |
| `forgejo.actions_incus_client_key_b64` | string | — | `""` | `static` | `FORGEJO_ACTIONS_INCUS_CLIENT_KEY_B64` | no | no | yes | yes | `container_recreate` | Base64 restricted-project Incus TLS client key |
| `forgejo.actions_incus_endpoint` | string | — | `""` | `static` | `FORGEJO_ACTIONS_INCUS_ENDPOINT` | no | no | no | yes | `container_recreate` | HTTPS endpoint of the independent Incus/KVM host |
| `forgejo.actions_incus_profile` | string | `length: 1..63`; `pattern: ^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$` | `anas-forgejo-runner` | `static` | `FORGEJO_ACTIONS_INCUS_PROFILE` | no | no | no | yes | `container_recreate` | Restricted one-job Runner VM profile name |
| `forgejo.actions_incus_server_cert_b64` | string | — | `""` | `static` | `FORGEJO_ACTIONS_INCUS_SERVER_CERT_B64` | no | no | yes | yes | `container_recreate` | Base64 pinned Incus server certificate |
