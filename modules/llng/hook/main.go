...
func afterStart(module string, env map[string]string) []dockerCopy {
	...
	container := env["CONTAINER_PREFIX"] + "llng"
	out := []dockerCopy{}
	for _, app := range splitCSV(env["APPS_LIST"]) {
		prefix := "APPS_LIST__" + strings.ToUpper(app) + "__"
		path := env[prefix+"LOGO_PATH"]
		if path == "" {
			continue
		}
		out = append(out, dockerCopy{
			Source:      path,
			Container:   container,
			Destination: "/usr/share/lemonldap-ng/portal/htdocs/static/common/apps/",
		})
	}
	return out
}
...
func calcLLNG(e map[string]string, _ string, secrets *secretStore) error {
	return identityCalc("LLNG")(e, secrets)
}
func identityCalc(prefix string) func(map[string]string, *secretStore) error {
	return func(e map[string]string, secrets *secretStore) error {
		...
		// environment: consumers are in this module's dependency closure, so
		// anything published here reaches their .env files. The private key is
		// set in render_env instead, where the environment is module-local.
...
func moduleIdentity(prefix string, e map[string]string, _ string) error {
	e["DB_HOST"] = e[prefix+"_DB_HOST"]
	e["DB_POST"] = e[prefix+"_DB_PORT"]
	e["DB_USER"] = e[prefix+"_DB_USERNAME"]
	e["DB_PASSWORD"] = e[prefix+"_DB_PASSWORD"]
