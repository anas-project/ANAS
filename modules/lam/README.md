...
`Admins` 在这里是 LAM 登录授权组，不会自动授予域管理员权限。用户进入 LAM 后能执行的
...
the default server-profile name, not a username. Group membership authorizes LAM
login but does not itself grant domain-administration rights; Samba AD groups and
