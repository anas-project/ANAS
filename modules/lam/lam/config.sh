...
for name in LAM_ADMIN_PASSWORD LAM_LANGUAGE SAMBA_DC_ADMIN_GROUP_DN SAMBA_DC_BASE_COMPUTERS_DN SAMBA_DC_BASE_DN SAMBA_DC_BASE_GROUPS_DN SAMBA_DC_BASE_USERS_DN SAMBA_DC_DOMAIN SAMBA_DC_LDAP_BIND_DN SAMBA_DC_LDAP_BIND_PASSWORD SAMBA_DC_LDAPS_SERVER_URL TZ; do
  eval 'present=${'"$name"'+x}'
  if [ "$present" != x ]; then
