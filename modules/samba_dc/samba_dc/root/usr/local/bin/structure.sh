...
# Application capability groups. They are gated on the same switch as the rest
# of the organizational structure: a deployment that manages its own directory
# layout gets no ANAS-generated OUs at all, and that has to stay true for these.
#
# Unlike APP_*, there is deliberately no aggregate "all capabilities" group.
# APP_all exists because "let this person into every application" is a coarse
# wish people really have; "give this person every feature of every module" is
# not -- it is the fine-grained half of the model, and an aggregate would
# silently grant the capabilities of modules enabled later.
if [ "$SAMBA_DC_CREATE_STRUCTURE" == "true" ] && [ -n "$ANAS_IDENTITY_CAPABILITY_GROUPS" ]; then
  echo "Create capability ou & groups"
  create_ou "OU=Groups" $SAMBA_DC_BASE_DN "Groups"
  create_ou "OU=Cap" "OU=Groups,$SAMBA_DC_BASE_DN" "Application capability grants generated by ANAS"
  CAP_BASE="OU=Cap,OU=Groups"
  for capability in $(echo "$ANAS_IDENTITY_CAPABILITY_GROUPS" | tr "," "\n")
  do
    [ -z "$capability" ] && continue
    create_group "CAP_$capability" "$CAP_BASE" "Grants the $capability capability inside its application; does not grant sign-in"
  done
fi
