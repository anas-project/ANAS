...
func TestTraefikLocalAdminRotationWritesVerifiedBcrypt(t *testing.T) {
	...
	verifyTraefikDashboard = func(url, username, password string) error { return nil }
	defer func() { verifyTraefikDashboard = originalVerify }()
	dir := t.TempDir()
	if err := os.Mkdir(filepath.Join(dir, "dynamic"), 0700); err != nil {
		t.Fatal(err)
	}
	path := filepath.Join(dir, "dynamic", "dashboard-auth.yml")
