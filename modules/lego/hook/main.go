...
func calcLego(e map[string]string, _ string, _ *secretStore) error {
	...
	// Issuer-neutral contract for consumers. LEGO_* stays this module's private
	// vocabulary: naming the implementation in something every other module
	...
	// below, matching the convention the existing modules already use.
	...
	// which, so no module branches on the certificate source.
