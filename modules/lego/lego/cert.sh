...
if [ -z "${LEGO_PROVIDER_CODE:-}" ]; then
  echo "no DNS provider is configured for the ACME DNS-01 challenge; set modules.lego.config.dns_provider" >&2
  exit 1
fi

mkdir -p /certs/certs1000 "$OUT"

# DNS vendor credentials arrive namespaced to this module (LEGO_TENCENTCLOUD_*)
